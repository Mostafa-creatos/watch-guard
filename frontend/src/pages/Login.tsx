...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data || t('Invalid username or password'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-slate-50 dark:bg-slate-950 transition-colors duration-300 px-4 relative overflow-hidden">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-2">
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-2 w-32 rounded-xl bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
            <div className="py-1">
              <button onClick={() => setLanguage('en')} className="w-full text-left px-4 py-2 text-sm text-slate-750 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-750 font-medium">English</button>
              <button onClick={() => setLanguage('fr')} className="w-full text-left px-4 py-2 text-sm text-slate-750 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-750 font-medium">Français</button>
              <button onClick={() => setLanguage('ar')} className="w-full text-left px-4 py-2 text-sm text-slate-750 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-750 font-medium font-arabic">العربية</button>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-md bg-white/70 dark:bg-slate-905/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 p-8 rounded-3xl shadow-2xl relative z-10">
        ...
        {error && (
          <div className="mb-6 p-4 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/30 text-red-600 dark:text-red-400 text-sm font-medium">
            {error}
          </div>
        )}
        ...
        <p className="mt-8 text-center text-sm text-slate-500 dark:text-slate-450 font-medium">
          {t('noAccount')}{' '}
